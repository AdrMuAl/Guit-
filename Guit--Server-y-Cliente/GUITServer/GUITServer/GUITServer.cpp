...
void handleAddCommand(sqlite3* DB, const vector<string>& params, SOCKET clientSocket) {
    ...
    vector<char> fileContents(fileContent.begin(), fileContent.end());

    // Insertar archivo en la db
    string sql = "INSERT INTO Archivos (repositorio_id, nombre, contenido) VALUES ((SELECT id FROM Repositorios ORDER BY id DESC LIMIT 1), ?, ?);";
    sqlite3_stmt* stmt;
    ...
    sqlite3_bind_blob(stmt, 2, fileContents.data(), fileContents.size(), SQLITE_TRANSIENT);
...
void handleCommitCommand(sqlite3* DB, const string& message, SOCKET clientSocket) {
    ...
    string sql = "SELECT id, contenido FROM Archivos WHERE repositorio_id = (SELECT id FROM Repositorios ORDER BY id DESC LIMIT 1);";
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(DB, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    for (const std::pair<int, std::vector<char>>& file : files) {
        int id = file.first;
        const std::vector<char>& content = file.second;
        string checksum = calculateMD5(content);

        // Insertar commit
        string insertCommitSql = "INSERT INTO Commits (repositorio_id, mensaje, checksum) VALUES ((SELECT id FROM Repositorios ORDER BY id DESC LIMIT 1), ?, ?);";
        ...
        // versión del archivo
        string insertVersionSql = "INSERT INTO Versiones (archivo_id, commit_id, contenido, checksum) VALUES (?, (SELECT id FROM Commits ORDER BY id DESC LIMIT 1), ?, ?);";
...
void handleRollbackCommand(sqlite3* DB, const string& file, const string& commitId, SOCKET clientSocket) {
    string sql = "SELECT contenido FROM Versiones V INNER JOIN Archivos A ON V.archivo_id = A.id WHERE A.nombre = ? AND V.commit_id = ?;";
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(DB, sql.c_str(), -1, &stmt, NULL);
